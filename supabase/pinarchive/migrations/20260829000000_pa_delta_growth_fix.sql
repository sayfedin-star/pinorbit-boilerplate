-- Migration: 20260829000000_pa_delta_growth_fix.sql
-- Description: Enforce monotonic lifetime metrics for saves/repins and fix delta_saves / delta_repins calculations

-- 1. pa_account_pins_page with explicit ORDER BY in row_number and non-negative delta clamping
  ...
  WITH base_pins AS (
    ...
    LEFT JOIN LATERAL (
      ...
      SELECT
        ...
        CASE
          WHEN count(*) >= 2 THEN
            GREATEST(0, max(CASE WHEN rnum = 1 THEN os2.saves END) - max(CASE WHEN rnum = 2 THEN os2.saves END))
          ELSE 0
...
-- 2. pa_ingest_pin_batch with monotonic GREATEST saves/repins protection
...
BEGIN
  ...
  WITH upserted AS (
    ...
    ON CONFLICT (workspace_id, pin_id) DO UPDATE SET
      ...
      END,
      annotations = CASE
        WHEN jsonb_array_length(excluded.annotations) > 0 THEN excluded.annotations
