-- Migration: 20260830000000_pinarchive_fastcron_tokens_rls.sql
-- Project: Project 4 (PinArchive Data Lake)
-- Description: Authenticated tenant-isolation RLS policies for pinarchive_fastcron_tokens
...
CREATE POLICY "Members read pinarchive_fastcron_tokens" 
  ON public.pinarchive_fastcron_tokens FOR SELECT TO authenticated 
  USING (public.is_workspace_member(workspace_id));

DROP POLICY IF EXISTS "Admins insert pinarchive_fastcron_tokens" ON public.pinarchive_fastcron_tokens;
CREATE POLICY "Admins insert pinarchive_fastcron_tokens" 
  ON public.pinarchive_fastcron_tokens FOR INSERT TO authenticated 
  WITH CHECK (public.is_workspace_admin(workspace_id));

DROP POLICY IF EXISTS "Admins update pinarchive_fastcron_tokens" ON public.pinarchive_fastcron_tokens;
CREATE POLICY "Admins update pinarchive_fastcron_tokens" 
  ON public.pinarchive_fastcron_tokens FOR UPDATE TO authenticated 
  USING (public.is_workspace_admin(workspace_id)) 
  WITH CHECK (public.is_workspace_admin(workspace_id));

DROP POLICY IF EXISTS "Admins delete pinarchive_fastcron_tokens" ON public.pinarchive_fastcron_tokens;
CREATE POLICY "Admins delete pinarchive_fastcron_tokens" 
  ON public.pinarchive_fastcron_tokens FOR DELETE TO authenticated 
  USING (public.is_workspace_admin(workspace_id));
