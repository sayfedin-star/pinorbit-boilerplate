...
export const DELETE: APIRoute = async ({ params, locals }) => {
  ...
  try {
    ...
    const result = await deleteWorkspaceToken(workspaceId, id, 'pinarchive', envWithClient);
    if (!result.success) {
...
export const PATCH: APIRoute = async ({ request, params, locals }) => {
  ...
  try {
    await assertWorkspaceAccess(client, workspaceId, user.id, 'admin');
    const pinAdmin = dbClients.getPinArchive(runtimeEnv);

    if (body.is_default === true) {
      await pinAdmin
        .from('pinarchive_fastcron_tokens')
        .update({ is_default: false })
        .eq('workspace_id', workspaceId);

      const { error } = await pinAdmin
        .from('pinarchive_fastcron_tokens')
